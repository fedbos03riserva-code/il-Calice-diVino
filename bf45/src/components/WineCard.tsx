...
export default function WineCard({ wine, onAddToCart, compact }: WineCardProps) {
  const { t, addToCart, toggleSaveWine, isSaved, getWineRating } = useApp();
    ...
    <div className="group bg-cream-50 rounded-xl border border-cream-200 overflow-hidden hover:shadow-lg hover:border-gold-300 transition-all duration-300">
      <Link to={`/wine/${wine.id}`}>
        ...
        <div className="relative h-40 bg-gradient-to-b from-cream-100 to-cream-200 flex items-end justify-center pb-4">
          <div className={`w-12 h-28 rounded-t-lg ${typeColors[wine.tipo] || "bg-bordeaux-700"} group-hover:scale-105 transition-transform duration-300 relative`}>
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3 h-6 bg-bordeaux-950 rounded-t" />
            <div className={`absolute top-7 left-1/2 -translate-x-1/2 ${typeText[wine.tipo]} text-[8px] font-serif rotate-180 whitespace-nowrap`} style={{ writingMode: "vertical-rl" }}>
      ...
      <button onClick={() => toggleSaveWine(wine)} className="absolute top-3 right-3 p-1.5 rounded-full bg-cream-50/80 hover:bg-cream-50 transition-colors">
        <Heart className={`w-4 h-4 ${isSaved(wine.id) ? "fill-bordeaux-600 text-bordeaux-600" : "text-bordeaux-400"}`} />
      </button>

      {/* Info */}
      <div className="p-4">
        <Link to={`/wine/${wine.id}`}>
          <h3 className="font-serif text-base font-semibold text-bordeaux-950 line-clamp-2 leading-tight hover:text-bordeaux-700 transition-colors">{wine.nome}</h3>
        </Link>
        <p className="text-xs text-bordeaux-600 mt-1">{wine.regione} &middot; {wine.continente}</p>

        {/* Rating */}
        {count > 0 && (
          <div className="flex items-center gap-1 mt-1.5">
            <Star className="w-3.5 h-3.5 fill-gold-400 text-gold-400" />
            <span className="text-xs font-medium text-bordeaux-700">{avg}</span>
            <span className="text-xs text-bordeaux-400">({count})</span>
          </div>
        )}
        ...
        {!compact && (
          <div className="flex flex-wrap gap-1 mt-2">
            {wine.profilo_aromatico.slice(0, 3).map((a) => (
              <span key={a} className="text-[10px] px-1.5 py-0.5 rounded-full bg-cream-200 text-bordeaux-700">{a}</span>
            ))}
          </div>
        )}
